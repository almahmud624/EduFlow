...
const TopCourses = () => {
    ...
    return (
        <div className="py-7">
            <h1 className="text-6xl font-bold mt-12 capitalize">Our top courses</h1>
            <div className="min-h-screen flex justify-center items-center py-20">
                <div className="md:px-4 md:grid md:grid-cols-2 lg:grid-cols-3 gap-7 space-y-4 md:space-y-0">
                    {
                        courses?.slice(0, 4).map(course => <div key={Math.random()} className="max-w-sm bg-white px-6 pt-6 pb-2 rounded-xl shadow-lg transform  transition duration-500">
                            ...
                            <h1 className="mt-4 text-gray-800 text-xl font-semibold cursor-pointer text-left">{course.courseName}</h1>
                            <h5 className="mt-4 text-gray-500 text-md font-medium text-left border-l-4 border-red-800 py-0 pl-2">{course.categories}</h5>
